...
Out_Data_File = "/Users/Wm1/Desktop/CoLAG_Research/YangPython/YangOUTDATA_4_28_16_1_5mil_.001.csv"
...
def csvOutput(File, run, cnt, G, W, P):
    Gout =""
    Pout =""
    Wout =""
    for i in range(n):
    ...
    for i in range(n):
      Pout += str(round(P[i],15))+","

    outStr = str(run)+","+str(cnt)+","+str(bin2Dec(G))+","+"'"+Gout+","+Wout+","+Pout+"\n"
    File.write(outStr)
...
def run():
    ...
    for runNum in range(trials):
      ...
      Pset  = [-1 for i in range(n)] # Pset[i] is numSents when Wcurr[i] reaches threshold
 ...
 #         s = chooseSentUnif()
          ...
          for i in range(n):
             if Pset[i] == -1:
                 CONVERGED = False
                 if (1-Wcurr[i] < threshold  or Wcurr[i] < threshold): 
                    Pset[i] = numSents
  

      

      csvOutput(OUTDATA, runNum, numSents, Gcurr , Wcurr, Pset)      
